...
{
    public static string key = "P@ssw0rd12345678";
    ...
    public static string EncryptDES(this string encryptString, string encryptKey)
    {
        try
        {
            var rgbKey = Encoding.UTF8.GetBytes(encryptKey.Substring(0, 16));
            var rgbIV = Keys;
        ...
        {
            throw new System.Exception("密码加密异常" + ex.Message);
        }
    }
    ...
    {
        var rgbKey = Encoding.UTF8.GetBytes(decryptKey.Substring(0, 16));
        var rgbIV = Keys;
        var inputByteArray = Convert.FromBase64String(decryptString.Replace('_', '+').Replace('~', '/'));
        using (var DCSP = Aes.Create())
        {
            using (var mStream = new MemoryStream())
            {
                ...
                {
                    var inputByteArrays = new byte[inputByteArray.Length];
                    cStream.Write(inputByteArray, 0, inputByteArray.Length);
                    cStream.FlushFinalBlock();
